"""
Fork of `train_decoder_lora_primitives.py`, pointed at the much wider
`training/build_benchcorpus_slice.py` data (62,000 train examples across 17
sources, real better-jev-bench corpus data for noul/score instead of one
narrow dataset each -- see that script's docstring for exactly what changed
and why). No mechanism changes from the primitives script: same fixed
ascending-scale order for `score` rows, same masking/OOM-skip/temperature
machinery, all already proven correct at this point in the lineage. Only the
default `--data-dir`/`--output-dir` differ, so this can't accidentally read
13a.8's data or overwrite its checkpoint.

Run: `uv run python3 -u -m training.train_decoder_lora_benchcorpus --grad-checkpointing --batch-size 4 --grad-accum-steps 16`
"""
...
def main():
    ...
    ap.add_argument("--output-dir", default=str(REPO_ROOT / "checkpoints" / "ekvachan-decoder-qwen-benchcorpus"))
    ...
    manifest = {
        ...
        "architecture": "decoder-lora-restricted-logit-benchcorpus",
