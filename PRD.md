...
### 13a.38 Small (~0.5B) same-family model: pipeline audited for size-generality, two real gaps found and fixed, no training script forked (2026-09-26, session with no GPU access)

The owner asked for a ~0.5B-parameter model from the same Qwen family, trained on the same data (or more), benchmarked the same way (JevBench/jabr-v2/ViZDoom), on the expectation that a much smaller model gets latency down further (possibly sub-millisecond) and makes free-tier hosting/demoing easier.

**What this session actually did, since it has no GPU to train anything**: audited whether the existing pipeline can take a different `--base-model` at all, rather than assuming either "it just works" or "it needs a rewrite." It mostly already does -- verified by reading the code, not by trusting a docstring:
- `training/train_decoder_lora_general.py` already takes `--base-model` as a plain CLI flag (default `Qwen/Qwen3.5-4B`), applies to both text and vision data, and writes the base model name into its output manifest.
- `benchmarks/common/backends.py`'s `RoutingModelBenchmarkBackend` (`--backend routing-decoder`, PRD 13a.32) already takes `--checkpoints-dir`/`--text-adapter`/`--use-cuda-graphs`/`--base-model` as CLI flags wired through `benchmarks/common/harness.py`, and `benchmarks/vizdoom/run.py` reuses that same harness unchanged -- so JevBench, jabr-v2, and ViZDoom all already support pointing at an arbitrary checkpoint directory with zero new benchmark code.
- `serve/inference.py`'s backends (`DecoderMultischemaBackend`/`DecoderVisionMultischemaBackend`) already read `base_model` from each checkpoint's own `manifest.json` rather than hardcoding it, and `training/probe_vision_path.py` (PRD 5.2b) already takes `--base-model` too -- so the exact same probe that validated the 4B model's vision path (config/auto-class mapping, param split, LoRA target-module compatibility, chat template, image-token cost, the last-column masking trick, and optionally a real bf16 forward pass with load time/VRAM) can be pointed at a candidate small model directly, with no new probe script needed. Writing a near-duplicate of that probe would have repeated exactly the mistake `train_decoder_lora_general.py`'s own docstring describes fixing (forking instead of parameterizing) -- so none was written.

**Two real gaps found in that audit, both fixed here (small, additive, backward-compatible -- default behavior unchanged when nothing new is set)**:
1. `serve/inference.py`'s `load_default_router()` (what `serve/server.py` calls with zero arguments) had no way to point the *live-serving* path at a different checkpoints root -- only `RoutingDecoderModel.__init__` itself took a `checkpoints_dir` argument, and nothing upstream exposed it. Without this, there was no way to serve or latency-test a small-model checkpoint side by side with the production 4B checkpoints without either overwriting them or editing code. Fixed: `load_default_router()` now also accepts `checkpoints_dir`, falling back to a new `EKVACHAN_CHECKPOINTS_DIR` env var, falling back to the unchanged default (`REPO_ROOT/"checkpoints"`) when neither is set.
2. `train_decoder_lora_general.py`'s manifest recorded `base_model` and `init_adapter` but never the `--data-dir` that actually trained a given checkpoint, nor any real measured parameter count -- meaning no committed manifest, on its own, can answer "what data trained this" (has to be reconstructed from memory or by following `init_adapter` chains by hand) or "is this really a 0.5B model" (has to be taken on the strength of the `--base-model` string, exactly the kind of unverified claim this project's own evidence discipline argues against). Fixed: the manifest now also records `data_dir`, `n_params_total`, and `n_params_trainable` (measured directly off the loaded model, not inferred from the base-model name).

Both edits verified with `python3 -m py_compile` only, same constraint as every other piece of code in this project written without GPU access.

**What is deliberately NOT done or claimed here, left for the GPU-side agent**:
- **Whether a 0.5B, vision-capable checkpoint actually exists under some real HF repo id in the "Qwen3.5" family is not verified by this session** -- no internet or HF access. This must be checked for real, first, via `training/probe_vision_path.py --base-model <candidate-id>` (add `--gpu-forward` for a real load + forward pass) before any training run is started. If the candidate model has no vision-capable config/auto-class mapping (the probe's own section 2/3 output says so directly), the fallback is a text-only run (`AutoModelForCausalLM`, the `DecoderChoiceModel`/older frozen-script path) rather than assuming vision support exists at every size in the family.
- **The exact multi-stage data curriculum that produced the current best (`stage3`) 4B checkpoint is not centrally recorded anywhere before this fix** (that is precisely gap #2 above) -- older checkpoints' manifests predate the `data_dir` field and won't have it. The GPU-side agent must reconstruct the chain by following each checkpoint's own `manifest.json["init_adapter"]` pointer back stage by stage (`stage3` → whatever it continued from → ... → the first, non-continued run), cross-checked against the `training/build_*_slice.py` scripts' own hardcoded `out_dir`/source paths (e.g. `build_stage3_slice.py` continues from `checkpoints/ekvachan-decoder-qwen-wide` using `wide_slice_continue_capped/train` plus new go_emotions rows) -- not re-derived by this session, which cannot see `data/processed/` or the real checkpoints at all.
- **Latency expectation, stated honestly rather than assumed**: a much smaller model's own forward pass will almost certainly be substantially faster. But PRD 13a.24 measured that a real, non-trivial fraction of this project's end-to-end request latency on the 4B model is spent outside the pure forward pass (tokenization/processing, Python dispatch, request handling) -- overhead that does not shrink just because the model does. A large, real win is the reasonable expectation; **"sub-millisecond end-to-end latency" is this session's inference from the parameter-count ratio, not a measured claim, and should not be repeated as fact until PRD 13a.33's own methodology (real JevBench-shaped requests, p50/p95, eager vs. graph) is re-run against the small model's own checkpoint and manifest.**

**Handoff for the GPU-side agent**: see the message accompanying this commit for the exact command sequence (compatibility probe → curriculum reconstruction → training → full benchmark re-run → latency re-measurement), since PRD.md is the evidence record, not the run-by-run instruction sheet.
