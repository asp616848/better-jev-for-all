...
- [x] **`score`/`noul` training run complete** (2026-09-23, 71m54s, `checkpoints/ekvachan-decoder-qwen-primitives`, `results/ekvachan-decoder-qwen-primitives.train-manifest.json`) — PRD 13a.8. Real numbers: `choice` 92.02% (regression check, barely moved), **`noul` 88.80%** (looks real on a first pass), **`score` 75.40%** (honest weak spot — worst Brier in the table by a wide margin, 0.355 vs 0.11-0.16 for everything else; not yet disentangled whether that's task difficulty or the untested fixed-letter-order design). CLINC150 zero-shot choice regression check: 95.73% vs 13a.6's 96.10% baseline — noise-level difference, primitive mix did not hurt existing choice generalization. Still a proxy metric (argmax-over-letters), not `noul`'s real float output or `score`'s real scale-position output — that's still open, see below.
