...
- [x] **Part A shipped + measured** (2026-09-25, PRD 13a.25) — conditional `set_adapter()` skip in `predict_choice` (verified `active_adapter` string on the real object; redundant set cost 8.74ms = real-switch cost, PEFT never short-circuits): in-server total -9 to -10ms both cases; routing re-check passes both arms (501/200, bit-identical probs). Dispatch decomposed directly: pydantic ~2us, json ~11us, threadpool+starlette hop ~5ms, network/client ~4.5ms — async rewrite declined (saves <=5ms, needs run_in_threadpool for blocking torch+lock).
- [x] **Part B int8: fails, not shipped, no 4-bit** (2026-09-25, PRD 13a.26) — `--decoder-load-in-8bit` (QLoRA-style, bnb 0.50.2, default off): accuracy parity holds (JevBench +0.86pp, jabr-v2 +0.11pp) but int8 is **4.7x slower** (72.6ms -> 342.8ms p50) — killed on its own objective. CLINC150 gate n/a for benchcorpus by construction (26-cap vs 151-way). 4 evidence manifests committed.

- [x] **Kernels + compile round: overhead confirmed, levers exhausted, <50ms missed** (2026-09-25, PRD 13a.27) — Step 0: sdpa optimal, fla Triton kernel engaged in serving, conv fallback priced at ~2-3ms (not the lever); profiler: ~22ms/forward fla Python-wrapper + ~14ms/2800 kernel launches vs ~20ms real compute. Step 1: reduce-overhead unbuildable (g++ 7.5 vs C++20 wall, overrides ignored), default-mode compiles but -3ms then +9ms (SLOWER) — do not ship. No repo code changed; 13a.25 routing check stands.

- [x] **vLLM prototype: equivalent, routing works, NOT faster — NO-GO on latency** (2026-09-25, PRD 13a.28) — isolated venv (vLLM 0.30.0, cu130): guided-choice over code letters is bit-identical to the restricted-logit read on 2/5 cases (≤0.017 noise rest), vision adapter routes with maxabs 0.006 vs serving, zero switch penalty. But plain prefill+1 ≈91ms p50 (≈eager) and the logprob read we'd ship costs **174ms p50** — prefill-bound pattern gets none of vLLM's decode-throughput wins. Section 7 server stays the target. No repo code touched.

- [x] **Manual CUDA graphs: GO, -30ms (-42%), first sub-50ms p50** (2026-09-25, PRD 13a.29) — raw CUDAGraph capture/replay prototype (no repo code touched): replay 41.73/51.86ms vs same-run eager 72.02/93.93ms. 0.125 logit noise traced to bf16 tiling (cosine-1.0 with pad noise), ≤0.006 at probability level, argmax same 5/5. Buckets incl. 588-opt@8192 (warmup-before-capture required); copy_-switch 1.45ms (vs PEFT ~8ms), verified vs vision-eager (0.094) with restore. Routing re-check holds (501/200). Ship = separate decision (bench gate + E2E still owed).
- [x] **CUDA-graph integration shipped, flag-gated off by default** (2026-09-25, PRD 13a.30/13a.31) — `serve/inference.py` gained `_CudaGraphRunner` (`CUDA_GRAPH_BUCKETS`, dedicated scratch tensors so an `ensure_adapter()` failure can never corrupt the eager path, fail-closed: one failure permanently disables the runner for the process) and `RoutingDecoderModel(use_cuda_graphs=...)` / `EKVACHAN_USE_CUDA_GRAPHS`, off by default. Written blind (no GPU access that session), then GPU-verified and fixed for real: the real `AutoProcessor` output includes `mm_token_type_ids` (not just `input_ids`/`attention_mask` as first assumed) and padding now uses the tokenizer's real `pad_token_id`. 50-item JevBench correctness gate: **96.0% match, both modes**. E2E: graph **45.41ms p50 / 56.17ms p95** vs eager **85.04ms p50 / 91.35ms p95**.
- [x] **`RoutingModelBenchmarkBackend` added so JevBench/jabr-v2/ViZDoom test the real serving object** (2026-09-25, PRD 13a.32) — every prior benchmark run used a separate, independently-implemented model-loading path (`DecoderMultischemaBackend`), never `serve.inference.RoutingDecoderModel` itself. New `benchmarks/common/backends.py::RoutingModelBenchmarkBackend` wraps `RoutingDecoderModel` directly (base64-encodes `image_path` before delegating); wired into `benchmarks/common/harness.py` as `--backend routing-decoder` with `--checkpoints-dir`/`--text-adapter`/`--use-cuda-graphs`; `benchmarks/vizdoom/run.py` picks it up unchanged. Written blind, stub-verified (28/28 checks across this and prior stub tests) — not yet run against a real checkpoint.
- [x] **Full pre-launch validation against `routing-decoder`, README numbers corrected** (2026-09-25, PRD 13a.33) — JevBench (231/231) and jabr-v2 (944/944), eager vs CUDA-graph, both on the actual serving object: **JevBench 70.99% both modes, jabr-v2 85.49% both modes**, 3 + 9 item disagreements, max prob diff <0.063 across all 1175 items — graphs are semantically preserving. (An initial run read 68.83%/84.22% off a stale `vision`-slot symlink pointing at an old 26-option checkpoint instead of `stage3`; caught, fixed, re-run to the numbers above before trusting them — see 13a.33.) ViZDoom Defend the Center (`von` rubric): eager 8.75 kills vs graph 8.125 kills; Health Gathering matched exactly (32.85s). `bjb evaluate` full 14-task sweep reproduced the 13a.20 stage3 baseline (Generality 78.94 / Intelligence 66.39 / Calibration 88.64) once pointed at the same corrected symlink. E2E on real JevBench-shaped requests: **graph 111.67ms p50 / 131.22ms p95 vs eager 130.72ms p50 / 155.90ms p95** (~15% real-world speedup — lower than the 13a.31 synthetic-prompt numbers because real items are longer). README's "How we compare to the field" and latency sections updated to these numbers.
- [x] **CUDA graphs flipped ON by default in production** (2026-09-25, PRD 13a.34, decided by the project owner) — `RoutingDecoderModel.__init__`'s `use_cuda_graphs` default now resolves to `True` unless `EKVACHAN_USE_CUDA_GRAPHS=0` (or `use_cuda_graphs=False`) is set explicitly; previously the reverse. Basis: PRD 13a.33's full-corpus gate (JevBench 231/231, jabr-v2 944/944, `bjb evaluate`, ViZDoom, all against the real `RoutingDecoderModel` serving object) found accuracy identical within noise and a real ~15% E2E speedup on real request shapes (112ms vs 131ms p50). README updated to state the new default and its real production latency.
- [x] **GPU hardware name redacted from public docs/code/evidence** (2026-09-25, PRD 13a.35, project owner's pre-launch request) — every authored mention of the specific GPU model replaced with generic language across `PRD.md`/`STATUS.md`/`training/vision_class_swap_control.py`; two evidence-bundle manifests had `device_name` explicitly redacted (not silently overwritten). Real values still exist in git history and outside this repo. Flags a direct conflict with PRD 14 Q1's own reproducibility stance, left open rather than resolved.
- [ ] **Hugging Face publishing scaffolded, not yet run** (2026-09-25, PRD 13a.35, superseded by 13a.37 below) — original Docker-based ZeroGPU scaffold. Superseded, not deleted-and-forgotten: kept here for history since 13a.35's model-card/upload-script groundwork is still valid, only `hf_space/`'s own deployment mechanics changed.
- [ ] **HF Space rebuilt: no Docker, minimum-cost hardware brief** (2026-09-25, PRD 13a.37) — `hf_space/Dockerfile` deleted, `sdk: docker`→`sdk: gradio` + `requirements.txt`; `app.py` restructured so `@spaces.GPU` wraps only the actual forward pass (not input parsing/base64 work), model loads once at cold start on CPU and moves to `"cuda"` exactly once on first real request (never per-request), CUDA graphs forced off for this Space specifically (untested on T4/ZeroGPU). `scripts/publish_to_huggingface.py` now vendors the exact minimal source set `app.py` imports (`serve/`, `benchmarks/common/`, `training/decoder_lora_lib.py`, `eval/`) directly into the Space repo instead of relying on a build-time git clone. Full VRAM/load-time/latency-estimate/billing/pause report in PRD 13a.37 — real measured load time (6.2-6.8s) and VRAM (9.51/9.60GB) carried over from existing evidence bundles; **per-request latency on T4 is an explicit, unverified extrapolation**, not a benchmarked number, and **bf16-on-T4 support is a flagged, untested risk** to check on first real deploy. Written without HF/GPU access, never executed.
- [x] **README rewritten for public launch, stale partial-coverage table demoted** (2026-09-25, PRD 13a.36) — headline table now leads with the real full-coverage stage3 numbers (JevBench 231/231, jabr-v2 944/944, verified against the actual committed manifests, `is_complete_benchmark_score: true` both), Brier/ECE added; the old 2026-09-23 partial-attempt numbers (139/231, 387/944) moved to a labeled, collapsed "superseded" section instead of leading the README. **Open gap found, not yet closed**: no committed manifest anywhere in `results/` has `backend: "routing_decoder"` — 13a.33's full-coverage validation claim rests on `decoder_vision_multischema`-backend manifests, not a run through the new `RoutingModelBenchmarkBackend` wrapper the way that section describes. Numbers are very likely still correct (same mechanism, same weights), but the specific "measured through the new in-process wrapper" claim isn't independently evidenced yet — worth a real `--backend routing-decoder` run with its own committed manifest next GPU session.
...
- [x] **Hardware recorded** (2026-09-23): single datacenter-class GPU, ample VRAM headroom, real driver/compute-capability recorded internally (kept out of this public doc by policy) — see PRD 14 Q1.
...
Anchor: this server's real Qwen3.5-4B LoRA runs took 65–75 min for 24k examples / 1 epoch. Single datacenter-class GPU, ample VRAM headroom.
