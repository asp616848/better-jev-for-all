...
- [ ] **Hugging Face publishing scaffolded, not yet run** (2026-09-25, PRD 13a.35) — `hf_model/README.md` (model card), `hf_space/{app.py,Dockerfile,README.md}` (ZeroGPU Gradio demo, clones this repo at a pinned commit and runs the real `RoutingDecoderModel`), `scripts/publish_to_huggingface.py` (the actual upload script). Written without HF/GPU access, never executed. Real next steps: run the publish script on the GPU box, manually set the Space's Hardware to ZeroGPU, verify the Docker build and an end-to-end request, confirm `snapshot_download`'s layout matches what `RoutingDecoderModel` expects.
