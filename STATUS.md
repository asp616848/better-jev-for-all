...
| 2 | **Fine-tune: base** — **in progress** 2026-09-23 (`checkpoints/ekvachan-decoder-qwen-benchcorpus`, log `~/ekvachan/benchcorpus_run1.log`): 62,000 train examples (up from 13a.8's 24,000), 17 distinct sources (up from 4) via `training/build_benchcorpus_slice.py` — the real better-jev-bench corpus (55k public export, CLINC150 excluded to preserve the zero-shot regression check) plus the NLI/DBpedia-14 continuity anchor | ~3-3.5 hrs, in progress | data pulled from #1 |
