...
- [ ] **`serve/inference.py` + `serve/server.py` only serve the old fixed-3-way encoder** — never updated for the decoder, despite it being the chosen architecture (PRD 14 Q4). Verified 2026-09-23: zero references to the decoder, LoRA, or Qwen anywhere under `serve/`. This blocks everything that needs a live server against the real model, and it is the single largest gap between what this repo *claims* and what it *serves*. Scope note: this is not a small task — it needs adapter loading, the restricted-logit read, and generalizing `predict_choice` from a hard-coded 3-label list to an arbitrary ≤26-option set, plus a latency re-measurement (the 27–36 ms in PRD 13a.3 is an *encoder* number and will not carry over to a 4B decoder).
- [ ] **Training/eval manifests are not committed anywhere** — `checkpoints/` is gitignored (`git ls-files checkpoints/` → 0 files), so the manifests behind the four headline numbers (86.32 / 92.25 / 96.10 / 98.5) exist **only on `abhijeet-labgpu`**. PRD 8.2 and dev-guidelines rule 10 commit to an evidence bundle in `results/` for every run; the *benchmark* runs comply, the *training/eval* runs do not. Fix: copy each `checkpoints/<run>/manifest.json` to `results/<run>.train-manifest.json` and commit (the weights stay gitignored — it's a few KB of JSON). Until then, a server failure loses the provenance for every number in PRD 13a.
- [ ] **Hardware is still unrecorded** (PRD 14 Q1 flags this and it is still open) — no evidence bundle, manifest, or PRD section names the actual GPU model/VRAM these timings and OOM findings were measured on. Cheap to fix, and PRD 8.2's evidence discipline is weaker without it.
- [ ] Untracked `sdk/python/uv.lock` sitting in the working tree — either commit it or add it to `.gitignore`; dev-guidelines rule 11 wants `git status` clean before work ends.
...
|---|---|---|---|
| 1 | Commit training/eval manifests to `results/`; record the GPU model | none | — (minutes; closes the PRD 8.2 evidence gap) |
| 2 | Wire decoder support into `serve/` (adapter load + restricted-logit read + arbitrary ≤26-option `choice`), then re-measure latency | minutes, to benchmark | — (largest real dev task on this list) |
| 3 | Build `score` + `noul` decode formats and training data, then train | ~1–2 hrs | data design first; **the real unlock for the 92/231 + 557/944 unattempted items** |
